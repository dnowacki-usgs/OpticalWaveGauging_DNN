...
if __name__ == "__main__":
    ...
    parser.add_argument("-c", "--config", required=True, help="Name of config file")
    ...
    train_df, valid_df = train_test_split(
        new_df, test_size=test_size, random_state=2018, stratify=new_df["category"]
    )
    ...
    for batch_size in [128]:  # [16,32,64,128]:
        ...
        for arch in archs:
            ...
            weights_path = get_weights_path(
                input_csv_file, category, counter, imsize, batch_size, num_epochs
            )
            ...
            reduceloss_plat = ReduceLROnPlateau(
                monitor="val_loss",
                factor=factor,
                patience=5,
                verbose=1,
            ...
            OWG.compile(optimizer="rmsprop", loss="mse", metrics=[mae_metric])  # adam
            ...
            if category == "zscore":
                ...
                pred_Y = (
                    div
                    * np.squeeze(
                        OWG.predict(test_X, batch_size=batch_size, verbose=True)
                    )
                    + mean
                )
                if (prc_lower_withheld > 0) | (prc_upper_withheld > 0):
                    pred_extreme_Y = (
                        div
                        * np.squeeze(
                            OWG.predict(ex_X, batch_size=batch_size, verbose=True)
                        )
                        + mean
                    )
            else:
            ...
            ax1.legend()
            base_dir = Path.cwd() / f"im{imsize}" / "res" / f"{num_epochs}epoch"
            if category == "H":
                ax1.set_xlabel("Actual H (m)")
                ax1.set_ylabel("Predicted H (m)")

                out_dir = base_dir / "H" / f"model{counter}" / f"batch{batch_size}"
                out_dir.mkdir(parents=True, exist_ok=True)

                filename = (
                    f"im{IMG_SIZE[0]}_waveheight_model{counter}_"
                    f"{num_epochs}epoch{batch_size}batch_{input_csv_file}.png"
                )
                ...
                out_dir = base_dir / "T" / f"model{counter}" / f"batch{batch_size}"
                out_dir.mkdir(parents=True, exist_ok=True)

                filename = (
                    f"im{IMG_SIZE[0]}_waveperiod_model{counter}_"
                    f"{num_epochs}epoch{batch_size}batch_{input_csv_file}.png"
                )
